                                    ...
                                    @foreach ($siswa as $x)
                                        <tr>
                                            ...
                                            <td class="align-middle-center" style="display: flex; align-items: center; justify-content: center;">
                                                ...
                                                <button type="button" class="btn btn-danger btn-sm" data-toggle="tooltip" data-original-title="Hapus siswa" data-nis="{{ $x->nis }}" data-bs-toggle="modal" data-bs-target="#hapusSiswa{{ $x->nis }}">
                                                    <i class="bi bi-trash"></i>
                                                </button>
                                                <form id="deleteForm{{ $x->nis }}" method="POST" action="{{ route('admin.siswa.destroy', ['nis' => $x->nis]) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <div class="modal fade" id="hapusSiswa{{ $x->nis }}" tabindex="-1" aria-labelledby="hapusSiswaLabel{{ $x->nis }}" aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h1 class="modal-title fs-5" id="hapusSiswaLabel{{ $x->nis }}"> Hapus Siswa</h1>
                                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                                </div>
                                                                <div class="modal-body">
                                                                    Apakah Anda yakin ingin menghapus siswa dengan NIS {{ $x->nis }}?
                                                                </div>
